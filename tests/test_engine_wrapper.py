...
def test_engine_path_set_and_valid():
    ...
    with (
        patch.dict(
            os.environ, {"ENGINE_PATH": "/valid/path/to/engine"}, clear=True
        ),
...
def test_engine_path_set_but_invalid():
    ...
    with (
        patch.dict(
            os.environ, {"ENGINE_PATH": "/invalid/path/to/engine"}, clear=True
        ),
...
def test_engine_path_unset_some_fallback_vars():
    ...
    with (
        patch.dict(
            os.environ,
            {"ENGINE_OS": "windows", "ENGINE_BINARY": "stockfish.exe"},
            clear=True,
        ),
        patch("os.path.isfile", return_value=True),
        patch("os.access", return_value=True),
...
def test_engine_path_unset_empty_binary():
    ...
    with (
        patch.dict(os.environ, {"ENGINE_BINARY": ""}, clear=True),
        pytest.raises(
            EngineBinaryError, match="Engine binary name cannot be empty"
        ),
    ):
        _get_engine_path()
...
def test_engine_path_unset_fallback_not_found():
    ...
    with (
        ...
        pytest.raises(EngineBinaryError, match="Engine binary not found"),
    ):
        _get_engine_path()


def test_fallback_not_executable():
    """Test when fallback path exists but is not executable."""
    with (
        patch.dict(os.environ, {}, clear=True),
        patch("os.path.isfile", return_value=True),
        patch("os.access", return_value=False),
        pytest.raises(
            EngineBinaryError, match="Engine binary is not executable"
        ),
    ):
        _get_engine_path()
