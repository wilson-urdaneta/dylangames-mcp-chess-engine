...
async def run_server(logs_dir): # Depends on logs_dir fixture if used
    ...
    try:
        ...
        if process.poll() is not None:
             raise RuntimeError(f"Server process terminated unexpectedly with code {process.returncode}")
        ...
        if process and process.poll() is None:
            try:
                ...
                logger.info(f"Server process exited gracefully (code {process.returncode}).")
            except subprocess.TimeoutExpired:
                logger.warning("Server process did not exit gracefully, killing.")
            ...
            logger.info(f"Server process already terminated (code {process.returncode}).")
...
async def test_http_get_best_move(run_server): # Use the new fixture
    ...
    try:
        ...
        logging.info(f"Attempting to connect to SSE endpoint: {sse_endpoint_url}")
        async with sse_client(sse_endpoint_url, timeout=15.0) as streams: # Increased timeout
            ...
            async with ClientSession(*streams) as session:
                ...
                # Call the tool
                logging.info(f"Calling tool 'get_best_move_tool' with args: {tool_arguments}")
                ...
                assert hasattr(result, 'content'), "Result object lacks 'content' attribute"
                assert isinstance(result.content, list), "'result.content' is not a list"
                assert len(result.content) > 0, "'result.content' list is empty"
                assert isinstance(result.content[0], types.TextContent), "First content item is not TextContent"
                assert hasattr(result.content[0], 'text'), "Result content lacks 'text' attribute"
                ...
                try:
                    ...
                    assert len(result_data["best_move_uci"]) > 0 # e.g., 4 chars like 'e2e4'
                    print(f"Best move verified in JSON: {result_data['best_move_uci']}")
                except json.JSONDecodeError:
                    # If not JSON, check if the expected key is just in the string
                    assert "best_move_uci" in result_text
                    print(f"Best move verified in text: {result_text}") # Adjust assertion if needed

    except Exception as e:
        import traceback
        traceback.print_exc() # Print full traceback for debugging
        pytest.fail(f"Failed to call tool via SSE: {type(e).__name__}: {str(e)}")
